	...
	if (isUseOfMesh)
	{
		for (int i = 0; i < model.getNumMeshes(); i++)
		{
			...
			for (int j = 0; j < mesh.getVertices().size(); j++)
			{
				...
				mesh.setVertex(j, ofVec3f(vert->x - meshVertsOffset.x, vert->y - meshVertsOffset.y, vert->z - meshVertsOffset.z));
...
void BodyPart::drawFaces()
{
    ...
    //ofRotate(qangle, qaxis.x, 0, qaxis.z);
    //ofRotate(qangle, qaxis.x, qaxis.y, 0);
    //ofRotate(qangle, 0, qaxis.y, qaxis.z);
    //ofRotate(qangle, 0, qaxis.y, qaxis.z);
...
//    texture->bind();
    ...
    if (isUseOfMesh)
	{
		...
		if (isVisible) mesh.drawWireframe();
